...
const { projectName, cssModules } = config
const date = new Date()
...
const embedPath = ENV === 'PROD' ? `${config.path[ENV]}/_BUILD/${ENV}` : config.path[ENV]
...
const project = `${config.path[ENV].split('T3Interactives/')[1]}`
